...
public class DetailTipVerticalPagerAdatper extends FragmentStatePagerAdapter {
    ...
    public DetailTipVerticalPagerAdatper(FragmentManager fm, int count, DetailTips.ResultData resultData) {
        super(fm);
        this.count = count;
        this.resultData = resultData;
        user = resultData.userInfo;
    ...
    public Fragment getItem(int position) {
        switch (position){
            case 0:
                return DetailCoverVerticalFragment.getInstance(BundleBuilder.create().with("tipFirstPage", resultData.userInfo).build());
            default:
